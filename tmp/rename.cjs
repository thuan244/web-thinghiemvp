const fs = require('fs');
const path = require('path');

function slugify(text) {
  // map Vietnamese characters
  let str = text.toLowerCase();
  str = str.replace(/à|á|ạ|ả|ã|â|ầ|ấ|ậ|ẩ|ẫ|ă|ằ|ắ|ặ|ẳ|ẵ/g, 'a');
  str = str.replace(/è|é|ẹ|ẻ|ẽ|ê|ề|ế|ệ|ể|ễ/g, 'e');
  str = str.replace(/ì|í|ị|ỉ|ĩ/g, 'i');
  str = str.replace(/ò|ó|ọ|ỏ|õ|ô|ồ|ố|ộ|ổ|ỗ|ơ|ờ|ớ|ợ|ở|ỡ/g, 'o');
  str = str.replace(/ù|ú|ụ|ủ|ũ|ư|ừ|ứ|ự|ử|ữ/g, 'u');
  str = str.replace(/ỳ|ý|ỵ|ỷ|ỹ/g, 'y');
  str = str.replace(/đ/g, 'd');
  // Some system don't handle quotes or strange chars well
  str = str.replace(/[^a-z0-9]/g, '-');
  // remove multiple hyphens
  str = str.replace(/-+/g, '-');
  // trim hyphens
  str = str.replace(/^-+/, '');
  str = str.replace(/-+$/, '');
  return str;
}

function processDirectory(dir) {
  let files;
  try { files = fs.readdirSync(dir); } catch(e) { return; }
  for (const file of files) {
    const oldPath = path.join(dir, file);
    const ext = path.extname(file);
    const base = path.basename(file, ext);
    let newName = slugify(base) + ext.toLowerCase(); // Lowercase the extension too
    
    // We shouldn't slugify empty names completely
    if (newName === ext.toLowerCase()) {
        newName = file.toLowerCase().replace(/[^a-z0-9\-.]/g, '-');
    }

    const newPath = path.join(dir, newName);
    
    if (oldPath !== newPath) {
      if (fs.existsSync(newPath)) {
        console.log(`Warning: target path ${newPath} already exists, skipping rename of ${oldPath}`);
        // If it's a directory, still process its contents
        if (fs.statSync(oldPath).isDirectory()) {
            processDirectory(oldPath);
        }
        continue;
      }
      fs.renameSync(oldPath, newPath);
      console.log(`Renamed: ${oldPath} -> ${newPath}`);
    }
    
    if (fs.statSync(newPath).isDirectory()) {
      processDirectory(newPath);
    }
  }
}

// Rename the root folder to phong-thi-nghiem first
const rootOld = path.join('public', 'Phòng thí nghiệm');
const rootNew = path.join('public', 'phong-thi-nghiem');

if (fs.existsSync(rootOld)) {
  if (!fs.existsSync(rootNew)) {
    fs.renameSync(rootOld, rootNew);
    console.log(`Renamed root: ${rootOld} -> ${rootNew}`);
  }
}

if (fs.existsSync(rootNew)) {
  processDirectory(rootNew);
}

// Now update ts file
let tsFile = path.join('src', 'data', 'phongThiNghiem.ts');
if (fs.existsSync(tsFile)) {
  let content = fs.readFileSync(tsFile, 'utf8');
  content = content.replace(/path:\s*'([^']+)'/g, (match, urlPath) => {
    const parts = urlPath.split('/').filter(Boolean);
    const newParts = parts.map(part => {
      const lastDot = part.lastIndexOf('.');
      if (lastDot !== -1 && lastDot > 0 && part.toLowerCase().endsWith('.pdf')) {
        const name = part.slice(0, lastDot);
        const ext = part.slice(lastDot);
        return slugify(name) + ext.toLowerCase();
      }
      return slugify(part);
    });
    return `path: '/${newParts.join('/')}'`;
  });
  fs.writeFileSync(tsFile, content);
  console.log('Successfully updated paths in phongThiNghiem.ts');
}
console.log('Done script.');
